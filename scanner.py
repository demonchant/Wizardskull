...
def get_klines(symbol, interval, limit=100):
    """Fetches candlestick data from Binance Futures with error handling."""
    try:
...
def scan_asset(asset):
    """Calculates Long and Short scores for an asset."""
    ...
    ls_ratio = get_long_short_ratio(asset)    price = asset_4h['close'].iloc[-1]
    
    # --- LONG SCORING (Max 10) ---
    long_score = 0
    if btc_4h['close'].iloc[-1] > calc_ema(btc_4h['close'], 20).iloc[-1]: long_score += 1
    if btc_1d['close'].iloc[-1] > calc_ema(btc_1d['close'], 20).iloc[-1]: long_score += 1
    if btc_3d['close'].iloc[-1] > calc_ema(btc_3d['close'], 20).iloc[-1]: long_score += 1
        
    macd, signal = calc_macd(btc_4h)
    if macd > signal: long_score += 1
        
    k, d, j = calc_kdj(btc_4h)
    if j > k and k > d: long_score += 1
        
    if asset_4h['close'].iloc[-1] > calc_ema(asset_4h['close'], 20).iloc[-1]: long_score += 1
        
    k, d, j = calc_kdj(asset_4h)
    if j > k and k > d: long_score += 1
        
    if funding < 0.01: long_score += 1
    if oi_delta > 0: long_score += 1
    if ls_ratio < 1.05: long_score += 1

    # --- SHORT SCORING (Max 10) ---
    short_score = 0
    if btc_4h['close'].iloc[-1] < calc_ema(btc_4h['close'], 20).iloc[-1]: short_score += 1
    if btc_1d['close'].iloc[-1] < calc_ema(btc_1d['close'], 20).iloc[-1]: short_score += 1
    if btc_3d['close'].iloc[-1] < calc_ema(btc_3d['close'], 20).iloc[-1]: short_score += 1
        
    if macd < signal: short_score += 1
    if j < k and k < d: short_score += 1
        
    if asset_4h['close'].iloc[-1] < calc_ema(asset_4h['close'], 20).iloc[-1]: short_score += 1
    if j < k and k < d: short_score += 1
        
    if funding > 0.01: short_score += 1
    if oi_delta > 0: short_score += 1
...
def main():
    ...
    for asset in ASSETS:
        try:
            ...
            if long_score >= 7:
                tp1_price = price * 1.0217  # +2.17%
                liq_price = price * 0.978   # Mock isolated liq price
            ...
            # CHECK FOR SHORT SIGNAL
                ...
                tp1_price = price * 0.9783  # -2.17%
                liq_price = price * 1.022   # Mock isolated liq price for short
                ...
                message = (
                    ...
                    f"🎯 TP1 — 100% at 46x (-2.17%) → `{tp1_price:.4f}`\n"
                    f"🎯 TP2 — Extended target\n"                    f"️ STOP LOSS — set below liq!\n"
                    f"💀 LIQ PRICE (isolated) — *~{liq_price:.4f}*\n\n"
                    f"🚨 *SL IS ABOVE LIQ PRICE — YOU WILL BE LIQUIDATED BEFORE SL TRIGGERS!*\n\n"
                    f"⚡ *Futures Intelligence:*\n"
                    f"📈 Funding Rates (8H): *{funding:.4f}%*\n"
                    f"📊 Open Interest Δ (24H): *{oi_delta:.2f}%*\n"
                    f" Liquidation Cluster: *{liq_status}*\n\n"
            ...
            # Sleep briefly to avoid hitting Binance API rate limits
            time.sleep(1)
